    ...
    **New Features:**
    ...
    def __init__(self):
        ...
        self.engine = create_engine(
            ...
            # ✅ REMOVED: connect_args that caused the time_zone error
            # connect_args are now handled in the URL itself
        )
        ...
        try:
            ...
            # Instead of raising, let's try a fallback connection
            try:
    ...
    def _fallback_connection_test(self):
        ...
        try:
            # Create a simpler engine for testing
            ...
            with test_engine.connect() as conn:
                ...
                if test_value == 1:
                    # If basic connection works, update our main engine
    ...
    def _build_enhanced_database_url(self) -> str:
        ...
        # ✅ FIXED: Only PyMySQL-compatible parameters (removed HTML entities)
        params = [
            "ssl_verify_cert=true",
            "charset=utf8mb4"
        ]
    ...
    def _test_enhanced_connection(self):
        ...
        with self.engine.connect() as conn:
            ...
            # Test TiDB version and capabilities
            version_result = conn.execute(text("SELECT VERSION()")).fetchone()
            logger.info(f"🗄️ Connected to: {version_result[0] if version_result else 'Unknown'}")
            
            # Test vector capabilities (if available)
            try:
                conn.execute(text("SELECT VEC_COSINE_DISTANCE('[1,2,3]', '[1,2,3]')"))
                logger.info("🧠 Vector operations supported")
            except Exception:
                logger.warning("⚠️ Vector operations not available (consider TiDB version upgrade)")
    ...
    async def init_database(self):
        ...
        def _init_sync():
            with self.engine.begin() as conn:
                try:
                    # ✅ FIXED: Use CREATE TABLE IF NOT EXISTS to prevent errors
                    ...
                    # ✅ FIXED: Vector index creation with proper error handling
                    try:
                        # First, try to drop any existing problematic indexes
                        drop_queries = [
                            "DROP VECTOR INDEX IF EXISTS idx_embedding ON research_papers",
                            "DROP VECTOR INDEX IF EXISTS idx_embedding_cosine ON research_papers",
                            "DROP VECTOR INDEX IF EXISTS idx_embedding_v2 ON research_papers"
                        ]
                        ...
                        conn.execute(text("""
                            CREATE VECTOR INDEX idx_embedding_v3 
                    ...
                    logger.info("✅ Enhanced database tables initialized with vector support")
    ...
    def _convert_enhanced_paper_from_db(self, paper_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'embedding' in paper_dict and paper_dict['embedding']:
            # Keep as TiDB vector format for vector operations
            pass
        ...
        def _get_filtered_sync():
            ...
            try:
                ...
                if filters.get('min_quality', 0) > 0:
                    conditions.append("context_quality_score >= :min_quality")
    ...
    async def cleanup_database(self, days_old: int = 30) -> Dict[str, int]:
        ...
        def _cleanup_sync():
            session = self.SessionLocal()
            try:
                # Clean up pending papers older than specified days
