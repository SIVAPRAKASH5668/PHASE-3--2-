...
class VectorOperations:
    ...
    async def update_paper_embedding(self, paper_id: int, embedding: np.ndarray) -> bool:
        """Update paper embedding in database"""
        try:
            ...
            try:
                ...
                result = session.execute(update_query, [formatted_vector, paper_id])
                session.commit()
            ...
            try:
                for paper_id, embedding in paper_embeddings:
                    try:
                        ...
                        # ✅ FIXED: Use SQLAlchemy parameter style
                        update_query = text("""
                            UPDATE research_papers 
                        ...
                        # ✅ FIXED: Use parameter dict instead of list
